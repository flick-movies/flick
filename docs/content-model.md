...
and movie genres. It returns the existing `PredictionResult`, including a 0–1
...
fields are accepted but currently unused for scoring.
...
`genre-v2`. Profiles are immutable; rebuild them when training data or settings
...
score = clamp(b + genre_weight * bounded_genre_component, 0, 5)
```

The default bound and genre weight are both 1. Missing or entirely unknown
genres produce the baseline and zero confidence. Debug's `raw_genre_component`
...
confidence = history_support * genre_support
```
...
have support. Disabling the genre weight or component bound returns zero
confidence and no reason signals.
...
Nonzero matched preferences produce genre `ReasonSignal` entries. `strength`
...
Version 2 defaults intentionally produce different scores; rebuild saved
...
ranking, diversity, group aggregation, and richer metadata signals remain
